...
export function useVoiceCall({ agentId, getToken, onCallEnd }) {
  ...
  const startCall = useCallback(async ({ emp, company, callLogs = [], productCtx = '', discoveryBlock = '', memoryBlock = '', worldCtx = '', dateCtx }) => {
    ...
    try {
      ...
      await conversation.startSession({
        ...sessionConfig,
        overrides: {
          agent: { prompt: { prompt: buildPersonaPrompt(emp, company, callLogs, productCtx, discoveryBlock, memoryBlock, worldCtx, dateCtx) }, firstMessage: personaFirstMessage(emp) },
