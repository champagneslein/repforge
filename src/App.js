      ...
      <div className="bg-[#EDEAE2] border-b border-[#D4CFC4] px-6 py-3 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
          ...
          {!simComplete ? (
            ...
            <button onClick={() => setTab("score")} className="bg-[#1A3A2A] text-white text-xs px-3 py-1.5 rounded-lg font-medium flex items-center gap-1">ÃÂ°ÃÂÃÂÃÂ View Final Score</button>
          )}
          <div className="text-xs text-[#1A3A2A] bg-[#DDD9D0] px-2 py-1 rounded">Jamie Doyle ÃÂÃÂ· Trainee AE</div>
        </div>
      </div>
      ...
      {tab === "crm" && (
        <div className="p-6">
            ...
            <>
              ...
              <div className="grid grid-cols-4 gap-4">
                {companies.filter(c => filterInd==="All" || c.industry===filterInd).map(c => {
                  ...
                  return (
                    <div key={c.id} onClick={() => { setSelCompany(c); setCrmView("company"); }} className="bg-white rounded-xl border border-[#D4CFC4] p-4 cursor-pointer hover:border-[#1A3A2A] hover:shadow-md transition-all group">
          ...
          {crmView === "company" && selCompany && (
            <>
              <div className="flex items-center gap-2 mb-5 text-sm">
                <button onClick={() => { setCrmView("accounts"); setSelCompany(null); }} className="text-[#1A3A2A] hover:underline font-medium">ÃÂ¢ÃÂÃÂ Accounts</button>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-1.5">
                      ...
                      <span className="text-gray-400 text-xs">ÃÂ°ÃÂÃÂÃÂ {selCompany.location}</span>
                    </div>
                  </div>
                </div>
              </div>
                      ...
                      return (
                        <tr key={emp.id} className="border-b border-[#EDE9E0] hover:bg-[#EDF5EE] cursor-pointer" onClick={() => { setSelEmp(emp); setCrmView("employee"); }}>
                          ...
                          <td className="px-4 py-3">
                            ...
                            {cs.emailStatus==="sent" && <span className="text-xs text-yellow-600 font-medium">ÃÂ¢ÃÂÃÂ³ Awaiting reply</span>}
                          ...
                          <td className="px-4 py-3">
                            ...
                            {cs.linkedinStatus==="pending" && <span className="text-xs text-yellow-600 font-medium">ÃÂ¢ÃÂÃÂ³ Pending</span>}
                          ...
                          <td className="px-4 py-3" onClick={e=>e.stopPropagation()}>
                            <div className="flex gap-1.5">
                              ...
                              <button onClick={() => { setEmailCompose({emp, company:selCompany}); setEmailDraft({subject:"",body:""}); }} className="bg-[#1A3A2A] hover:bg-[#2A5A3A] text-white text-xs px-3 py-1.5 rounded-lg font-medium">ÃÂ¢ÃÂÃÂ Email</button>
                              <button onClick={() => { setPlProfile(emp); setTab("prolink"); setPlView("profile"); }} className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs px-3 py-1.5 rounded-lg font-medium">ÃÂ°ÃÂÃÂÃÂ Profile</button>
                              <button onClick={() => activeCallId === emp.id ? endCall() : startCall(emp, selCompany)} disabled={activeCallId !== null && activeCallId !== emp.id} style={{marginLeft:'6px',padding:'4px 12px',borderRadius:'6px',border:'none',cursor:'pointer',fontSize:'13px',fontWeight:'600',background:activeCallId===emp.id?(callStatus==='active'?'#ef4444':'#f97316'):'#7c3aed',color:'white',opacity:activeCallId!==null&&activeCallId!==emp.id?0.4:1}}>{activeCallId === emp.id ? (callStatus === 'connecting' ? '📞 Connecting...' : '🔴 End Call') : '🎙️ AI Call'}</button>
                            </div>
                          </td>
                        </tr>
                      );
          ...
          {crmView === "employee" && selEmp && selCompany && (
            <>
              <div className="flex items-center gap-2 mb-5 text-sm">
                <button onClick={() => { setCrmView("accounts"); setSelCompany(null); setSelEmp(null); }} className="text-[#1A3A2A] hover:underline font-medium">ÃÂ¢ÃÂÃÂ Accounts</button>
                <span className="text-gray-300">/</span>
                ...
                <div className="w-64 flex-shrink-0 space-y-4">
                  <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
                    ...
                    <div className="mt-3 space-y-1 text-xs text-gray-500 text-left border-t border-gray-100 pt-3">
                      ...
                      {(state[selEmp.id]?.calls||0) > 0 && <div className="text-[#1A3A2A] font-medium pt-1">ÃÂ¢ÃÂÃÂ {state[selEmp.id].calls} call{state[selEmp.id].calls>1?"s":""} logged</div>}
                    </div>
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <button onClick={() => { setEmailCompose({emp:selEmp, company:selCompany}); setEmailDraft({subject:"",body:""}); }} className="bg-[#1A3A2A] hover:bg-[#2A5A3A] text-white rounded-lg py-2 text-sm font-medium">ÃÂ¢ÃÂÃÂ Email {selEmp.first}</button>
                    <button onClick={() => { setPlProfile(selEmp); setTab("prolink"); setPlView("profile"); }} className="bg-[#2A5A3A] hover:bg-[#1A3A2A] text-white rounded-lg py-2 text-sm font-medium">ÃÂ°ÃÂÃÂÃÂ View ProLink Profile</button>
                    ...
                    <div className="p-4 min-h-40 space-y-2">
                      {state[selEmp.id]?.emailThread.length === 0 ? (
                        ...
                        <div key={i} className={`p-3 rounded-lg text-xs leading-relaxed ${msg.from==="rep" ? "bg-[#EDF5EE] border border-[#B8D4BB] ml-8" : "bg-[#F2EFE8] border border-[#D4CFC4] mr-8"}`}>
                          <div className={`font-semibold mb-1 ${msg.from==="rep" ? "text-[#1A3A2A]" : "text-gray-800"}`}>{msg.from==="rep" ? "You" : `${selEmp.first} ${selEmp.last}`} ÃÂÃÂ· Day {msg.day}</div>
                          {msg.subject && <div className="font-medium text-gray-700 mb-0.5">Re: {msg.subject}</div>}
                          {msg.body}
                        </div>
                      ))}
                      {state[selEmp.id]?.emailStatus==="sent" && (
                        <div className="text-center py-2 text-xs text-yellow-600">ÃÂ¢ÃÂÃÂ³ Email sent ÃÂ¢ÃÂÃÂ reply expected around Day {state[selEmp.id].emailReplyDay || "?"}. Click Advance Day to progress.</div>)}
                ...
                <button onClick={async () => {
                  ...
                  {(state[selEmp.id]?.callLog||[]).length > 0 && (
                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mt-4">
                      <div className="px-4 py-3 border-b border-gray-100 font-semibold text-gray-700 text-sm">ÃÂ°ÃÂÃÂÃÂ Call Log ({state[selEmp.id].callLog.length})</div>
                          ...
                          <div key={i} className="px-4 py-3 flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-0.5">
                                ...
                                <span className="text-xs text-gray-400">ÃÂÃÂ· Day {log.day}</span>
                              </div>
          ...
          <div className="flex-1 p-6 overflow-auto">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <div><span className="font-semibold text-gray-800">{emp.first} {emp.last}</span><span className="text-gray-400 text-xs ml-2">{emp.title} ÃÂÃÂ· {company?.name}</span></div>
                        ...
                        <div className="mt-2 space-y-1.5">
                          {thread.map((msg,i) => (
                            <div key={i} className={`p-2.5 rounded-lg text-xs leading-relaxed ${msg.from==="rep" ? "bg-[#EDF5EE] border border-[#B8D4BB] ml-6" : "bg-[#F2EFE8] border border-[#D4CFC4] mr-6"}`}>
                              <div className={`font-semibold mb-0.5 ${msg.from==="rep" ? "text-[#1A3A2A]" : "text-gray-800"}`}>{msg.from==="rep" ? "You" : emp.first} ÃÂÃÂ· Day {msg.day}</div>
                              {msg.body}
                            </div>
                          ))}
                        </div>
            ...
            {plView === "sent" && (
              <>
                ...
                {sent.length === 0 ? <div className="text-center py-16 text-gray-400"><div className="text-4xl mb-3">ÃÂ°ÃÂÃÂÃÂ¤</div><div>No emails sent yet.</div></div>
                : sent.map(({emp, company, thread}) => (
                  <div key={emp.id} className="bg-white rounded-xl border border-gray-200 p-4 mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="font-medium text-gray-800 text-sm">{emp.first} {emp.last} <span className="text-gray-400 font-normal">ÃÂÃÂ· {company?.name}</span></div>
                        <div className="text-gray-500 text-xs">{thread[0]?.body?.slice(0,80)}...</div>
                      </div>
                      <div>
                        {state[emp.id]?.emailStatus==="replied" ? <span className="text-xs text-green-600 font-medium">ÃÂ¢ÃÂÃÂ Replied</span> : <span className="text-xs text-yellow-600">ÃÂ¢ÃÂÃÂ³ Awaiting</span>}
                      </div>
                    </div>
                  </div>
                ))}
                ...
                {allEmps.filter(e => state[e.id]?.emailStatus==="sent").map(emp => {
                  ...
                  return (
                    <div key={emp.id} className="bg-white rounded-xl border border-gray-200 p-3 mb-2 flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="font-medium text-gray-800 text-sm">{emp.first} {emp.last} <span className="text-gray-400 font-normal">ÃÂÃÂ· {company?.name}</span></div>
                        <div className="text-gray-400 text-xs">{seniorityLabels[emp.seniority]} ÃÂÃÂ· Sent Day {cs.emailThread[0]?.day}</div>
                      </div>
        ...
        <div className="flex min-h-screen">
          ...
          <div className="flex-1 max-w-2xl mx-auto p-6">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <div className="text-gray-500 text-xs">{emp.title} ÃÂÃÂ· {company?.name}</div>
                            </div>
                            <div>
                              ...
                              {cs.linkedinStatus === "ignored" && <span className="text-gray-400 text-xs">ÃÂ¢ÃÂÃÂ</span>}
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            {plView === "network" && (
              <div>
                ...
                {pendingConnections > 0 && (
                  <div className="mb-5">
                    <div className="font-semibold text-gray-700 text-sm mb-3">ÃÂ¢ÃÂÃÂ³ Pending Requests ({pendingConnections})</div>
                    ...
                    {allEmps.filter(e => state[e.id]?.linkedinStatus==="pending").map(emp => {
                      ...
                      return (
                        <div key={emp.id} className="bg-white rounded-xl border border-gray-200 p-3 mb-2 flex items-center gap-3">
                          ...
                          <div className="flex-1"><div className="font-medium text-gray-800 text-sm">{emp.first} {emp.last}</div><div className="text-gray-400 text-xs">{emp.title} ÃÂÃÂ· {company?.name}</div></div>
                          <span className="text-yellow-600 text-xs">Pending ÃÂÃÂ· Expected Day {state[emp.id].linkedinReplyDay}</span>
                        </div>
                      );
                    })}
                  </div>
                )}
                <div className="font-semibold text-gray-700 text-sm mb-3">ÃÂ°ÃÂÃÂÃÂ Connections ({connections.length})</div>
                    ...
                    return (
                      <div key={emp.id} className="bg-white rounded-xl border border-gray-200 p-3 flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-gray-400 text-xs">{emp.title} ÃÂÃÂ· {company?.name} ÃÂÃÂ· {company?.location}</div>
                        </div>
                        <div className="flex gap-1.5">
            ...
            {plView === "profile" && plProfile && (() => {
              ...
              return (
                <div>
                  <button onClick={() => setPlView("feed")} className="text-blue-500 text-sm hover:underline mb-4 block">ÃÂ¢ÃÂÃÂ Back to Feed</button>
                    ...
                    <div className="px-5 pb-5">
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <div className="text-gray-500 text-sm">{company?.name} ÃÂÃÂ· {company?.location}</div>
                          <div className="text-gray-400 text-xs mt-1">{company?.industry} ÃÂÃÂ· {company?.size}</div>
                          <div className="text-gray-400 text-xs mt-1 font-mono">ÃÂ°ÃÂÃÂÃÂ {getPhone(plProfile)}</div>
                        </div>
            ...
            {plView === "messages" && (
              <div>
                ...
                {connections.length === 0 ? (
                  <div className="text-center py-12 text-gray-400"><div className="text-4xl mb-3">ÃÂ°ÃÂÃÂÃÂ¬</div><div>No connections yet. Connect with people to message them.</div></div>
                ) : (
      ...
      {tab === "score" && (() => {
        ...
        return (
          <div className="p-6 max-w-3xl mx-auto">
            ...
            {/* Meeting list */}
            <div className="bg-white rounded-xl border border-[#D4CFC4] overflow-hidden">
              <div className="px-4 py-3 border-b border-[#D4CFC4] font-semibold text-gray-800 text-sm">ÃÂ°ÃÂÃÂÃÂ Meetings Booked ({meetingsBooked})</div>
              {meetingsBooked === 0 ? (
                ...
                return (
                  <div key={emp.id} className="px-4 py-3 border-b border-[#EDE9E0] flex items-center gap-3 last:border-0">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-gray-400 text-xs">{emp.title} ÃÂÃÂ· {company?.name}</div>
                    </div>
          ...
          <div className="bg-[#1A2A20] rounded-3xl shadow-2xl w-80 overflow-hidden">
            ...
            <div className="px-6 py-4 text-center min-h-28">
              ...
              {callPhase === "outcome" && callOutcome === "no-answer" && (
                <div>
                  <div className="text-gray-400 text-sm font-semibold mb-2">ÃÂ°ÃÂÃÂÃÂµ No Answer</div>
              ...
              {callPhase === "outcome" && (
                <>
                  ...
                  {(callOutcome === "connected" || callOutcome === "voicemail") && (
                    <button onClick={() => { endCall(); setEmailCompose({emp:callModal, company:getCompanyForEmp(callModal.id)}); setEmailDraft({subject:"Following up on our call", body:""}); }} className="w-14 h-14 rounded-full bg-[#1A3A2A] hover:bg-[#2A5A3A] border border-emerald-700 flex items-center justify-center text-2xl transition-colors">ÃÂ¢ÃÂÃÂ</button>
                  )}
                </>
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end justify-end z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-4 py-3 bg-[#1A3A2A] rounded-t-2xl flex items-center justify-between">
              ...
              <button onClick={() => setEmailCompose(null)} className="text-gray-400 hover:text-white text-lg">ÃÂÃÂ</button>
            </div>
            <div className="p-4">
              {emailCompose.emp ? (
