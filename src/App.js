  ...
  function advanceDay() {
    ...
    setState(prev => {
      ...
      Object.entries(next).forEach(([idStr, cs]) => {
        ...
        if (!emp) return;
        // Email replies
        if (cs.emailReplyDay && newDay >= cs.emailReplyDay && cs.emailStatus === "sent") {
          const replies = emailReplies[emp.seniority] || emailReplies["manager"];
          const reply = replies[id % replies.length];
          next[id] = { ...cs, emailThread: [...cs.emailThread, { from:"prospect", body:reply, day:newDay }], emailStatus:"replied" };
        }
        // LinkedIn accept
        if (cs.linkedinReplyDay && newDay >= cs.linkedinReplyDay && cs.linkedinStatus === "pending") {
          next[id] = { ...next[id], linkedinStatus: cs.willAcceptLinkedin ? "connected" : "ignored" };
        }
      });
      return next;
    });
    ...
    <div className="min-h-screen bg-[#F7F4EE]">
      ...
      <div className="bg-[#EDEAE2] border-b border-[#D4CFC4] px-6 py-3 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-xs text-[#1A3A2A] bg-[#DDD9D0] px-2 py-1 rounded">Jamie Doyle · Trainee AE</div>
        </div>
      </div>
        ...
        <div className="flex min-h-screen">
          ...
          <div className="flex-1 max-w-2xl mx-auto p-6">
            ...
            {plView === "profile" && plProfile && (() => {
              ...
              return (
                <div>
                  <button onClick={() => setPlView("feed")} className="text-blue-500 text-sm hover:underline mb-4 block">← Back to Feed</button>
                    ...
                    <div className="px-5 pb-5">
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <div className="text-gray-500 text-sm">{company?.name} · {company?.location}</div>
                          <div className="text-gray-400 text-xs mt-1">{company?.industry} · {company?.size}</div>
                          <div className="text-gray-400 text-xs mt-1 font-mono">📞 {getPhone(plProfile)}</div>
                        </div>
